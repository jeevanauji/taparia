        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active"><a href="{{ url('admin/product/edit/' . $productInfo->id) }}">Product Edit</a></li>
                </ol>
            </div>
        </div>
...
<section class="content">
    <form action="{{ url('admin/product/update/' . $productInfo->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="categoryId" id="categorySelect" class="form-control select2" style="width: 100%;" required="">
                                <option value="" disabled="" {{ $productInfo->categoryId == '' ? 'selected' : '' }}>Select Category</option>
                                @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ $productInfo->categoryId == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="subCategoryId" class="form-control select2" id="subCategoryId" style="width: 100%;">
                                <option value="" {{ $productInfo->subCategoryId == '' ? 'selected' : '' }}>Select Sub Category</option>
                                @foreach ($subCategories as $subCategory)
                                <option value="{{ $subCategory->id }}" {{ $productInfo->subCategoryId == $subCategory->id ? 'selected' : '' }}>{{ $subCategory->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="childSubCategoryId" class="form-control select2" id="childSubCategoryId" style="width: 100%;">
                                <option value="" {{ $productInfo->childSubCategoryId == '' ? 'selected' : '' }}>Select Child Sub Category</option>
                                @foreach ($childSubCategories as $childSubCategory)
                                <option value="{{ $childSubCategory->id }}" {{ $productInfo->childSubCategoryId == $childSubCategory->id ? 'selected' : '' }}>{{ $childSubCategory->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="productName" value="{{ $productInfo->productName }}" id="productName" class="form-control" placeholder="Enter Product Name" required="" />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <br />
                            <input type="radio" name="isNew" value="Y" id="Y" {{ $productInfo->isNew == 'Y' ? 'checked' : '' }} title="Yes" required="" />
                            Yes
                            <input type="radio" name="isNew" value="N" id="N" class="ml-5" {{ $productInfo->isNew == 'N' ? 'checked' : '' }} title="No" required="" />
                            No
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="oldProductCatalogue" value="{{ $productInfo->productCatalogue }}" />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="oldProductImage" value="{{ $productInfo->productImage }}" />
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <input type="hidden" name="productImageRowId[]" value="{{ $productImg->id }}" />
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('admin/product-image/delete/' . $productImg->id) }}" onclick="return confirm('Are you sure you want to delete this item?');" class="btn btn-danger btn-sm">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="productOverview" id="productOverview">{{ $productInfo->productOverview }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="productHighlighting">Special Features</label>
                            <textarea name="productHighlighting" id="productHighlighting">{{ $productInfo->productHighlighting }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="productSpecifications">Salient Features</label>
                            <textarea name="productSpecifications" id="productSpecifications">{{ $productInfo->productSpecifications }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
