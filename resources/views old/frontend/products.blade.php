...
<style type="text/css">
    ...
    .subcateLists h4 {
        ...
        padding-bottom: 10px;
        line-height: 1.3;
        order: -1; /* This ensures the heading stays at the top */
    }
...
<div class="hero-section">
    <div class="videobanner">
        <div class="image-banner">
            <img src="{{ asset('frontend/images/main_product_two.jpg') }}" alt="Banner">
        </div>
    </div>
    <div class="bannertxt">
        <div class="hero-text">
                    ...
                    <div class="filter-section">
                        ...
                        <div class="accordion" id="mainCategories">
                            ...
                            <div class="accordion-item acc_layer1 {{ request()->is('category/' . Str::slug($category->name)) ? 'active-category' : '' }}">
                                <h2 class="accordion-header d-flex" id="headingCategory{{ $category->id }}">
        ...
        <div class="row">
                        ...
                        <div class="product-list-box">
                            <div class="product-list-img">
                                @if ($subCategoryData['subCategory']->subCategoryImage)
                                    <img src="{{ url($subCategoryData['subCategory']->subCategoryImage) }}" class="img-fluid" alt="{{ $subCategoryData['subCategory']->name }}" />
                                @else
                                    <img src="{{ asset('frontend/images/default.jpg') }}" class="img-fluid" alt="Default Image" />
                                @endif
                            </div>
                        ...
                        <div class="product-list-box">
                            <div class="product-list-img">
                                @if ($childSubCategoryData['childSubCategory']->childSubCategoryImage)
                                    <img src="{{ url($childSubCategoryData['childSubCategory']->childSubCategoryImage) }}" class="img-fluid" alt="{{ $childSubCategoryData['childSubCategory']->name }}" />
                                @else
                                    <img src="{{ asset('frontend/images/default.jpg') }}" class="img-fluid" alt="Default Image" />
                                @endif
                            </div>
            ...
            @php } elseif (Str::startsWith(request()->path(), 'child-sub-category/')) { @endphp
                <div class="col-md-6 mt-3 mt-md-0 pb-3" style="width:100%">
                    <div class="product-list-box">
                        <div class="product-list-img">
                            <img src="{{ url($childSubCategoryData->childSubCategoryImage) }}" class="img-fluid" alt="{{ $childSubCategoryData->name }}" />
                        </div>
                        <div class="subcateLists">
                            <div>
                                <ul class="list-unstyled">
                        ...
                        <div class="product-list-box">
                            <div class="product-list-img">
                                @if ($categoryData->categoryImage)
                                    <img src="{{ url($categoryData->categoryImage) }}" class="img-fluid" alt="{{ $categoryData->name }}" />
                                @else
                                    <img src="{{ asset('frontend/images/default.jpg') }}" class="img-fluid" alt="Default Image" />
                                @endif
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.accordion').forEach(function (accordion) {
        accordion.addEventListener('show.bs.collapse', function (event) {
            // Agar koi panel already animation me hai to ignore karo
            if (event.target.classList.contains('collapsing')) {
                return;
            }
    ...
    // Tumhare original accordion buttons ka initialization
    ...
    // Page load par section title tak scroll karwana
    const sectionTitle = document.querySelector('.section-title');
    if (sectionTitle) {
        sectionTitle.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    // Category click karne par active class handle karna
    const categoryLinks = document.querySelectorAll('.main-title');
    categoryLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            // Pehle sab active classes hatao
            document.querySelectorAll('.accordion-item').forEach(item => {
                item.classList.remove('active-category');
            });
            document.querySelectorAll('.main-title').forEach(title => {
                title.classList.remove('active-category-link');
            });
            document.querySelectorAll('.subcategory-link').forEach(sub => {
                sub.classList.remove('active-subcategory');
            });

            // Jis item par click hua usko active karo
            const parentItem = this.closest('.accordion-item');
            if (parentItem) {
                parentItem.classList.add('active-category');
                this.classList.add('active-category-link');
            }

            // Agar subcategory link hai to parent category ko bhi active karo
            if (this.classList.contains('subcategory-link')) {
                const parentCategoryItem = this.closest('.acc_layer2').closest('.accordion-body').closest('.accordion-collapse').closest('.accordion-item');
                if (parentCategoryItem) {
                    parentCategoryItem.classList.add('active-category');
                    const parentCategoryLink = parentCategoryItem.querySelector('.main-title');
                    if (parentCategoryLink) {
                        parentCategoryLink.classList.add('active-category-link');
                    }
                }
            }
        });
    ...
    function nextSubcateSlide(index) {
        if (!subcateSliders[index]) {
            ...
            if (slides.length) {
                subcateSliders[index] = { current: 0, total: slides.length };
            } else return;
        }
    ...
    function nextProductSlide(index) {
        if (!productSliders[index]) {
            ...
            if (slides.length) {
                productSliders[index] = { current: 0, total: slides.length };
            } else return;
        }
    ...
    // Subcategory pagination event listeners
    document.querySelectorAll('.subcate-pagination .pagination-btn').forEach(button => {
        const idMatch = button.id.match(/subcate-(prev|next)-(\d+)/);
        if (idMatch) {
    ...
    // Product pagination event listeners
    document.querySelectorAll('.subcate-pagination .pagination-btn').forEach(button => {
        const idMatch = button.id.match(/product-(prev|next)-(\d+)/);
        if (idMatch) {
            const action = idMatch[1];
            const index = parseInt(idMatch[2]);
            button.addEventListener('click', () => {
    ...
    document.querySelectorAll('.subcate-slider').forEach(slider => {
        const idMatch = slider.id.match(/subcate-slider-(\d+)/);
        if (idMatch) {
    ...
    document.querySelectorAll('.subcate-slider').forEach(slider => {
        const idMatch = slider.id.match(/product-slider-(\d+)/);
        if (idMatch) {
            const index = parseInt(idMatch[1]);
            updateProductSlide(index);
        }
